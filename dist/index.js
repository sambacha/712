...
class EIP712Signer {
  ...
  _encodeDataTypeField(type, value) {
    ...
    if (type === "bytes") {
      return ["bytes32", ethers.utils.keccak256(value)];
    }
    if (type === "string") {
      return ["bytes32", ethers.utils.keccak256(Buffer.from(value, "utf8"))];
    }
    if (type === "uint256") {
    ...
    if (type.lastIndexOf("[]") === type.length - 2) {
      ...
      const abie = new ethers.utils.AbiCoder();
      return [
        "bytes32",
        ethers.utils.keccak256(abie.encode(encoded_array.map((elem) => elem[0]), encoded_array.map((elem) => elem[1])))
      ];
    }
    return [type, value];
  }
  ...
  _hashData(type, data) {
    return ethers.utils.keccak256(this._encodeData(type, data));
  }
  ...
  _hashType(type) {
    return ethers.utils.keccak256(Buffer.from(this._encodeType(type)));
  }
  ...
  _fromSigned(num) {
    return new BN__default["default"](Buffer.from(num.slice(2), "hex")).fromTwos(256);
  }
  _toUnsigned(num) {
  ...
  sign(privateKey, payload, verify = false) {
    return __async(this, null, function* () {
      ...
      switch (typeof privateKey) {
        case "string": {
          const sk = new ethers.utils.SigningKey(privateKey);
          const hashed_payload = ethers.utils.keccak256(Buffer.from(encoded_payload.slice(2), "hex"));
  ...
  verify(payload, signature, verify = false) {
    return __async(this, null, function* () {
      const encoded_payload = this.encode(payload, verify);
      const hashed_payload = ethers.utils.keccak256(Buffer.from(encoded_payload.slice(2), "hex"));
      return ethers.utils.recoverAddress(Buffer.from(hashed_payload.slice(2), "hex"), signature);
    });
  }
